...
- **从工作区仓库** —— 在 **设置 → 仓库** 里，对已保存的 GitHub 仓库点 **Sync skills**。Multica 会在默认分支上扫描常见 skill 目录（如 `.claude/skills`、`.cursor/skills`、`.openclaw/skills`、`.gemini/skills`、`.agent/skills` 等），以工作区 Skill 形式写入并带上来源信息，智能体在下次任务里与其它工作区 Skill 一样注入。v1 仅支持 **github.com**；会先跑 dry-run 预览，若会与同名已有 Skill 冲突需显式确认才会覆盖。
