...
Workspace skills come from five sources:
...
- **From workspace repository** — in **Settings → Repositories**, use **Sync skills** on a saved GitHub repository. Multica scans conventional skill paths (`.claude/skills`, `.cursor/skills`, `.openclaw/skills`, `.gemini/skills`, `.agent/skills`, and others) on the default branch and upserts them as workspace skills tagged with provenance so agents receive them on the next task (same injection path as other workspace skills). v1 supports **github.com** URLs only; run a dry-run preview before applying, and confirm when existing skills with the same name would be overwritten.
